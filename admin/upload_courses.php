    ...
    if ($file_ext !== 'csv') {
            ...
            while (($data = fgetcsv($handle)) !== FALSE) {
                if (count($data) >= 4 && !empty($data[0])) {
                    $courses[] = [
                        ...
                        'dept_code' => trim($data[3]),
                        'type' => !empty($data[4]) ? trim($data[4]) : 'theory'
                    ];
                }
            }
            fclose($handle);
        ...
        if (!empty($courses)) {
            ...
            foreach ($courses as $course) {
                ...
                $dept_result = $conn->query("SELECT department_id FROM departments WHERE department_code = '" . $conn->real_escape_string($course['dept_code']) . "'");
                
                if ($dept_result && $dept_result->num_rows > 0) {
                    $dept = $dept_result->fetch_assoc();
                    $dept_id = $dept['department_id'];
                    
                    // Check if course exists (by course_code and department_id)
                    $check = $conn->prepare("SELECT course_id FROM courses WHERE course_code = ? AND department_id = ?");
                    $check->bind_param('si', $course['code'], $dept_id);
                    $check->execute();
                    $result = $check->get_result();
                    
                    if ($result->num_rows > 0) {
                        // Update existing course
                        $stmt = $conn->prepare("UPDATE courses SET course_name = ?, credit_hours = ?, course_type = ? WHERE course_code = ? AND department_id = ?");
                        $stmt->bind_param('sissi', $course['name'], $course['credits'], $course['type'], $course['code'], $dept_id);
                        ...
                        // Insert new course
                        $stmt = $conn->prepare("INSERT INTO courses (course_code, course_name, credit_hours, department_id, course_type) VALUES (?, ?, ?, ?, ?)");
                        $stmt->bind_param('ssiis', $course['code'], $course['name'], $course['credits'], $dept_id, $course['type']);
                    ...
                    $errors[] = "Department code '{$course['dept_code']}' not found for course '{$course['code']}'";
                }
            }
            
            $message = "✅ Upload complete! Inserted/Updated: $inserted courses. Skipped: $skipped";
            if (!empty($errors)) {
...
$result = $conn->query("SELECT c.*, d.department_code, d.department_name FROM courses c JOIN departments d ON c.department_id = d.department_id ORDER BY d.department_code, c.course_code");
...
<head>
    ...
    <title>Upload Courses - Admin</title>
    ...
    <div class="container">
        ...
        <div class="glass-card">
            ...
            <form method="POST" enctype="multipart/form-data" id="uploadForm">
                ...
                <div style="text-align: center; margin-top: 30px; display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                    ...
                    <a href="download_template.php?type=courses" class="btn btn-secondary">
        ...
        <div class="glass-card">
            ...
            <div class="format-guide">
                ...
                <pre>course_code,course_name,credit_hours,department_code,course_type
...
CSE 3522,Database Management Systems Lab,1,CSE,lab</pre>
                <ul>
                    <li><code>course_code</code>: Unique course identifier (e.g., CSE 3521)</li>
                    <li><code>course_name</code>: Full course name</li>
                    <li><code>credit_hours</code>: Number of credits (usually 1-4)</li>
                    <li><code>department_code</code>: Must exist in Departments table</li>
                    <li><code>course_type</code>: Either "theory", "lab", or "project" (optional, defaults to "theory")</li>
                </ul>
                <p style="margin-top: 15px; color: #f68b1f; font-weight: 600;">
                    <i class="fas fa-exclamation-triangle"></i> Important: Department code must exist in Departments table first!
                </p>
            </div>
        </div>
        ...
        <div class="glass-card">
            <h2 style="color: var(--text-primary); margin-bottom: 20px; display: flex; align-items: center; gap: 10px; font-size: 1.5rem; font-weight: 600;">
                ...
                <span style="background: linear-gradient(135deg, #f68b1f, #fbbf24); color: white; padding: 4px 12px; border-radius: 20px; font-size: 0.9rem; margin-left: 10px;">
                    <?php echo count($existing_courses); ?>
                </span>
            </h2>
            ...
            <?php if (empty($existing_courses)): ?>
                <div class="empty-state">
                    ...
                    <p style="font-size: 1.1rem; margin-top: 10px;">No courses found. Upload a file to add courses.</p>
                </div>
            <?php else: ?>
                    ...
                    <table id="coursesTable">
                        ...
                        <tbody>
                            <?php foreach ($existing_courses as $course): ?>
                                <tr>
                                    <td><strong style="color: #f68b1f;"><?php echo htmlspecialchars($course['course_code']); ?></strong></td>
                                    <td><?php echo htmlspecialchars($course['course_name']); ?></td>
                                    <td><strong><?php echo $course['credit_hours']; ?></strong></td>
                                    <td><?php echo htmlspecialchars($course['department_code']); ?></td>
                                    <td>
                                        <?php
                                        $type = $course['course_type'];
                                        $badge_class = 'badge-theory';
                                        if ($type == 'lab') $badge_class = 'badge-lab';
                                        if ($type == 'project') $badge_class = 'badge-project';
                                        ?>
